...
// DeliverMailbox delivers an email to mailbox while recording mailboxDestined as
// the mailbox to which it would normally have been delivered (if it weren't for
// introbox and/or rejects). If mailboxDestined is empty, or equal to mailbox, the
// effect is a regular delivery to mailbox. Both mailboxes are created if
// necessary.
//
// Returns ErrOverQuota when account would be over quota after adding message.
//
...
// Message delivery, possible mailbox creation, and updated mailbox counts are
// broadcasted.
func (a *Account) DeliverMailbox(log mlog.Log, mailbox, mailboxDestined string, m *Message, msgFile *os.File) (rerr error) {
	var changes []Change
	...
	err := a.DB.Write(context.TODO(), func(tx *bstore.Tx) error {
		if mailboxDestined != "" && mailboxDestined != mailbox {
			mbDestined, chl, err := a.MailboxEnsure(tx, mailboxDestined, true, SpecialUse{}, &m.ModSeq)
			if err != nil {
