(() => {
	...
	var prewrap = (...l) => dom.span(style({ whiteSpace: "pre-wrap" }), l);
	var green = "#1dea20";
	var yellow = "#ffe400";
	var red = "#ff7443";
	var link = (href, anchorOpt) => dom.a(attr.href(href), attr.rel("noopener noreferrer"), anchorOpt || href);
	var crumblink = (text, path) => {
		return {
			text,
			path
		};
	...
	var crumbs = (...l) => {
		...
		const crumblink2 = (e) => typeof e === "string" ? prewrap(e) : dom.a(e.text, attr.href(e.path));
		return [
	...
	var footer = () => dom.div(style({ marginTop: "6ex", opacity: 0.75 }), link("https://www.xmox.nl", "mox"), " ", moxversion, " ", moxgoos, "/", moxgoarch, ", ", dom.a(attr.href("licenses.txt"), "licenses"));
	var age = (date, future, nowSecs) => {
		if (!nowSecs) {
	...
	var formatIPv6 = (b) => {
		...
		const hex = (v, skipzero) => (skipzero && (v >> 4 & 15) === 0 ? "" : hexchars[v >> 4 & 15]) + hexchars[v & 15];
	...
	var index = async () => {
		...
		return dom.div(
			...
			dom.div(style({ marginTop: ".5ex" }), dom.form(async function submit(e) {
				e.preventDefault();
				e.stopPropagation();
				dom._kids(cidElem);
	...
	var inlineBox = (color, ...l) => dom.span(style({
		display: "inline-block",
	...
	var accounts = async () => {
		...
		return dom.div(crumbs(crumblink("Mox Admin", "#"), "Accounts"), dom.h2("Accounts"), (accounts2 || []).length === 0 ? dom.p("No accounts") : dom.ul((accounts2 || []).map((s) => dom.li(dom.a(attr.href("#accounts/l/" + s), s), accountsDisabled?.includes(s) ? " (disabled)" : ""))), dom.br(), dom.h2("Add account"), dom.form(async function submit(e) {
			e.preventDefault();
			e.stopPropagation();
	...
	var accountloginattempts = async (accountName) => {
	...
	var renderLoginAttempts = (accountLinks, loginAttempts) => {
	...
	var RoutesEditor = (kind, transports, routes, save) => {
		...
		const render = () => {
			...
			let elem = dom.form(async function submit(e) {
				...
				render();
			})))), dom.tbody((routes || []).length === 0 ? dom.tr(dom.td(attr.colspan("5"), "No routes.")) : [], routeRows = (routes || []).map((r, index2) => {
	...
	var account = async (name) => {
		...
		return dom.div(crumbs(crumblink("Mox Admin", "#"), crumblink("Accounts", "#accounts"), name), config2.LoginDisabled ? dom.p(box(yellow, "Warning: Login for this account is disabled with message: " + config2.LoginDisabled)) : [], dom.h2("Addresses"), dom.table(dom.thead(dom.tr(dom.th("Address"), dom.th("Action"))), dom.tbody(Object.keys(config2.Destinations || {}).length === 0 ? dom.tr(dom.td(attr.colspan("2"), "(None, login disabled)")) : [], Object.keys(config2.Destinations || {}).map((k) => {
			...
			const close = popup(dom.h1("Disable account login"), dom.form(async function submit(e) {
				...
				close();
				window.location.reload();
			}, fieldset2 = dom.fieldset(dom.label(dom.div("Message to user"), loginDisabled = dom.input(attr.required(""), style({ width: "100%" })), dom.p(style({ fontStyle: "italic" }), "Will be shown to user on login attempts. Single line, no special and maximum 256 characters since message is used in IMAP/SMTP.")), dom.div(dom.submitbutton("Disable login")))));
	...
	var formatDuration = (v, goDuration) => {
		if (v === 0) {
			return "";
		}
		const is = (period2) => v > 0 && Math.round(v / period2) === v / period2;
		const format = (period2, s) => "" + v / period2 + s;
	...
	var domain = async (d) => {
		...
		return dom.div(crumbs(crumblink("Mox Admin", "#"), "Domain " + domainString(dnsdomain)), domainConfig.Disabled ? dom.p(box(yellow, "Warning: Domain is disabled. Incoming/outgoing messages involving this domain are rejected and ACME for new TLS certificates is disabled.")) : [], dom.ul(dom.li(dom.a("Required DNS records", attr.href("#domains/" + d + "/dnsrecords"))), dom.li(dom.a("Check current actual DNS records and domain configuration", attr.href("#domains/" + d + "/dnscheck")))), dom.br(), dom.h2("Client configuration"), dom.p("If autoconfig/autodiscover does not work with an email client, use the settings below for this domain. Authenticate with email address and password. ", dom.span("Explicitly configure", attr.title("To prevent authentication mechanism downgrade attempts that may result in clients sending plain text passwords to a MitM.")), " the first supported authentication mechanism: SCRAM-SHA-256-PLUS, SCRAM-SHA-1-PLUS, SCRAM-SHA-256, SCRAM-SHA-1, CRAM-MD5."), dom.table(dom.thead(dom.tr(dom.th("Protocol"), dom.th("Host"), dom.th("Port"), dom.th("Listener"), dom.th("Note"))), dom.tbody((clientConfigs.Entries || []).map((e) => dom.tr(dom.td(e.Protocol), dom.td(domainString(e.Host)), dom.td("" + e.Port), dom.td("" + e.Listener), dom.td("" + e.Note))))), dom.br(), dom.h2("DMARC aggregate reports summary"), renderDMARCSummaries(dmarcSummaries || []), dom.br(), dom.h2("TLS reports summary"), renderTLSRPTSummaries(tlsrptSummaries || []), dom.br(), dom.h2("Addresses"), dom.table(dom.thead(dom.tr(dom.th("Address"), dom.th("Account"), dom.th("Action"))), dom.tbody(Object.entries(localpartAccounts).map((t) => dom.tr(dom.td(prewrap(t[0]) || "(catchall)"), dom.td(dom.a(t[1], attr.href("#accounts/l/" + t[1]))), dom.td(dom.clickbutton("Remove", async function click(e) {
			...
			const elem = dom.form(style({ marginTop: "1ex" }), async function submit(e) {
				...
				addSeparatorView("");
			}), separatorsBox = dom.div(style({ display: "flex", flexDirection: "column", gap: ".25em" }), dom.div("(None)"))), dom.div(dom.span("\xA0"), dom.div(dom.submitbutton("Save")))));
	...
	var domainAlias = async (d, aliasLocalpart) => {
		...
		let addAddress;
		let delFieldset;
		return dom.div(crumbs(crumblink("Mox Admin", "#"), crumblink("Domain " + domainString(domain2.Domain), "#domains/" + d), "Alias " + aliasLocalpart + "@" + domainName(domain2.Domain)), dom.h2("Alias"), dom.form(async function submit(e) {
			e.preventDefault();
			e.stopPropagation();
			...
			window.location.reload();
		}, addFieldset = dom.fieldset(addAddress = dom.textarea(attr.required(""), attr.rows("1"), attr.placeholder("localpart@domain"), function focus() {
			addAddress.setAttribute("rows", "5");
		}), " ", dom.submitbutton("Add", style({ verticalAlign: "top" })))))))), dom.br(), dom.h2("Danger"), dom.form(async function submit(e) {
			e.preventDefault();
			e.stopPropagation();
	...
	var renderDMARCSummaries = (summaries) => {
		return [
	...
	var dmarcEvaluationsDomain = async (domain2) => {
		...
		const formatDKIMResults = (results) => results.map((r) => dom.div("selector " + r.Selector + (r.Domain !== domain2 ? ", domain " + r.Domain : "") + ": ", inlineBox(r.Result === "pass" ? "" : yellow, r.Result)));
		const formatSPFResults = (alignedpass, results) => results.map((r) => dom.div("" + r.Scope + (r.Domain !== domain2 ? ", domain " + r.Domain : "") + ": ", inlineBox(r.Result === "pass" && alignedpass ? "" : yellow, r.Result)));
		const sourceIP = (ip) => {
	...
	var utcDate = (dt) => new Date(Date.UTC(dt.getUTCFullYear(), dt.getUTCMonth(), dt.getUTCDate(), dt.getUTCHours(), dt.getUTCMinutes(), dt.getUTCSeconds()));
	var utcDateStr = (dt) => [dt.getUTCFullYear(), 1 + dt.getUTCMonth(), dt.getUTCDate()].join("-");
	var isDayChange = (dt) => utcDateStr(new Date(dt.getTime() - 2 * 60 * 1e3)) !== utcDateStr(new Date(dt.getTime() + 2 * 60 * 1e3));
	...
	var domainDMARC = async (d) => {
		...
		return dom.div(crumbs(crumblink("Mox Admin", "#"), crumblink("Domain " + domainString(dnsdomain), "#domains/" + d), "DMARC aggregate reports"), dom.p('DMARC reports are periodically sent by other mail servers that received an email message with a "From" header with our domain. Domains can have a DMARC DNS record that asks other mail servers to send these aggregate reports for analysis.'), dom.p("Below the DMARC aggregate reports for the past 30 days."), (reports || []).length === 0 ? dom.div("No DMARC reports for domain.") : dom.table(dom._class("hover"), dom.thead(dom.tr(dom.th("ID"), dom.th("Organisation", attr.title("Organization that sent the DMARC report.")), dom.th("Period (UTC)", attr.title("Period this reporting period is about. Mail servers are recommended to stick to whole UTC days.")), dom.th("Policy", attr.title("The DMARC policy that the remote mail server had fetched and applied to the message. A policy that changed during the reporting period may result in unexpected policy evaluations.")), dom.th("Source IP", attr.title("Remote IP address of session at remote mail server.")), dom.th("Messages", attr.title("Total messages that the results apply to.")), dom.th("Result", attr.title("DMARC evaluation result.")), dom.th("ADKIM", attr.title("DKIM alignment. For a pass, one of the DKIM signatures that pass must be strict/relaxed-aligned with the domain, as specified by the policy.")), dom.th("ASPF", attr.title("SPF alignment. For a pass, the SPF policy must pass and be strict/relaxed-aligned with the domain, as specified by the policy.")), dom.th("SMTP to", attr.title("Domain of destination address, as specified during the SMTP session.")), dom.th("SMTP from", attr.title("Domain of originating address, as specified during the SMTP session.")), dom.th("Header from", attr.title("Domain of address in From-header of message.")), dom.th("Auth Results", attr.title("Details of DKIM and/or SPF authentication results. DMARC requires at least one aligned DKIM or SPF pass.")))), dom.tbody((reports || []).map((r) => {
			...
			const sourceIP = (ip) => {
				const r2 = dom.span(ip, attr.title("Click to do a reverse lookup of the IP."), style({ cursor: "pointer" }), async function click(e) {
					e.preventDefault();
					try {
						const rev = await client.LookupIP(ip);
						r2.innerText = ip + "\n" + (rev.Hostnames || []).join("\n");
	...
	var domainDMARCReport = async (d, reportID) => {
	...
	var tlsrptResultsPolicyDomain = async (isrcptdom, domain2) => {
	...
	var renderTLSRPTSummaries = (summaries) => {
		return [
	...
	var domainTLSRPTID = async (d, reportID) => {
		const [report, dnsdomain] = await Promise.all([
	...
	var formatMTASTSMX = (mx) => {
		return mx.map((e) => {
			return (e.Wildcard ? "*." : "") + e.Domain.ASCII;
		}).join(", ");
	};
	var makeMTASTSTable = (items) => {
		if (items.length === 0) {
	...
	var dnsbl = async () => {
		...
		})), !Object.entries(ipZoneResults).length ? box(red, "No IPs found.") : [], dom.br(), dom.h2("DNSBL zones checked due to being used for incoming deliveries"), (usingZones || []).length === 0 ? dom.div("None") : dom.ul((usingZones || []).map((zone) => dom.li(domainString(zone)))), dom.br(), dom.h2("DNSBL zones to monitor only"), dom.form(async function submit(e) {
			e.preventDefault();
			e.stopPropagation();
	...
	var queueList = async () => {
		...
		const render = () => {
			...
			const ntbody = dom.tbody(dom._class("loadend"), msgs.length === 0 ? dom.tr(dom.td(attr.colspan("15"), "No messages.")) : [], msgs.map((m) => {
				return dom.tr(
					...
					dom.td(dom.clickbutton("Details", function click() {
						popupDetails(m);
					}))
				);
			}));
		...
		return dom.div(
			...
			dom.form(attr.id("holdRuleForm"), async function submit(e) {
				e.preventDefault();
				e.stopPropagation();
				const pr = {
					ID: 0,
			...
			(function() {
				...
				const renderHoldRules = () => {
					dom._kids(box2, !show ? dom.div("No hold rules. ", dom.clickbutton("Add", function click() {
						show = true;
						renderHoldRules();
					})) : [
			...
			dom.table(dom._class("hover"), style({ width: "100%" }), dom.thead(dom.tr(
				...
				}, dom.option("", attr.value("")), dom.option("Yes"), dom.option("No"))),
				...
				dom.td(),
				dom.td(),
				dom.td(filterTransport = dom.select(Object.keys(transports || {}).length === 0 ? style({ display: "none" }) : [], attr.form("queuefilter"), function change() {
					filterForm.requestSubmit();
				}, dom.option(""), dom.option("(default)"), Object.keys(transports || {}).sort().map((t) => dom.option(t)))),
				dom.td(
					attr.colspan("2"),
					style({ textAlign: "right" }),
					// Less content shifting while rendering.
					"Sort ",
					sortElem = dom.select(attr.form("queuefilter"), function change() {
						filterForm.requestSubmit();
					}, dom.option("Next attempt \u2191", attr.value("nextattempt-asc")), dom.option("Next attempt \u2193", attr.value("nextattempt-desc")), dom.option("Submitted \u2191", attr.value("submitted-asc")), dom.option("Submitted \u2193", attr.value("submitted-desc"))),
					" ",
			...
			})), dom.th("ID"), dom.th("Submitted"), dom.th("Account"), dom.th("From"), dom.th("To"), dom.th("Size"), dom.th("Attempts"), dom.th("Hold"), dom.th("Next attempt"), dom.th("Last attempt"), dom.th("Last error"), dom.th("Transport"), dom.th("Require TLS"), dom.th("Actions"))), tbody, dom.tfoot(dom.tr(dom.td(
				attr.colspan("15"),
			...
			dom.br(),
			dom.br(),
			dom.div(dom._class("unclutter"), dom.h2("Change selected messages"), dom.div(style({ display: "flex", gap: "2em" }), dom.div(dom.div("Hold"), dom.div(dom.clickbutton("On", async function click(e) {
	...
	var retiredList = async () => {
		...
		return dom.div(
			...
			dom.table(dom._class("hover"), style({ width: "100%" }), dom.thead(dom.tr(
				dom.td("Filter"),
				dom.td(filterSuccess = dom.select(attr.form("queuefilter"), function change() {
					filterForm.requestSubmit();
				}, dom.option(""), dom.option("Yes"), dom.option("No"))),
				...
				dom.td(
					...
					sortElem = dom.select(attr.form("queuefilter"), function change() {
						filterForm.requestSubmit();
					}, dom.option("Last activity \u2193", attr.value("lastactivity-desc")), dom.option("Last activity \u2191", attr.value("lastactivity-asc")), dom.option("Submitted \u2193", attr.value("submitted-desc")), dom.option("Submitted \u2191", attr.value("submitted-asc"))),
					" ",
					dom.submitbutton("Apply", attr.form("queuefilter")),
					" ",
					dom.clickbutton("Reset", attr.form("queuefilter"), function click() {
	...
	var hooksList = async () => {
		...
		return dom.div(
			...
			dom.table(dom._class("hover"), style({ width: "100%" }), dom.thead(dom.tr(dom.td(attr.colspan("2"), "Filter"), dom.td(filterSubmitted = dom.input(attr.form("hooksfilter"), style({ width: "7em" }), attr.title('Example: "<-1h" for filtering webhooks submitted more than 1 hour ago.'))), dom.td(), dom.td(), dom.td(), dom.td(filterAccount = dom.input(attr.form("hooksfilter"), style({ width: "8em" }))), dom.td(filterEvent = dom.select(
				...
				// Less content shifting while rendering.
				"Sort ",
				sortElem = dom.select(attr.form("hooksfilter"), function change() {
					filterForm.requestSubmit();
				}, dom.option("Next attempt \u2191", attr.value("nextattempt-asc")), dom.option("Next attempt \u2193", attr.value("nextattempt-desc")), dom.option("Submitted \u2191", attr.value("submitted-asc")), dom.option("Submitted \u2193", attr.value("submitted-desc"))),
				" ",
			...
			dom.br(),
			dom.br(),
			dom.div(dom._class("unclutter"), dom.h2("Change selected webhooks"), dom.div(style({ display: "flex", gap: "2em" }), dom.div(dom.div("Schedule next delivery attempt"), buttonNextAttemptSet("Now", 0), " ", dom.clickbutton("More...", function click(e) {
				e.target.replaceWith(dom.div(dom.br(), dom.div("Scheduled time plus"), dom.div(buttonNextAttemptAdd("1m", 1), " ", buttonNextAttemptAdd("5m", 5), " ", buttonNextAttemptAdd("30m", 30), " ", buttonNextAttemptAdd("1h", 60), " ", buttonNextAttemptAdd("2h", 2 * 60), " ", buttonNextAttemptAdd("4h", 4 * 60), " ", buttonNextAttemptAdd("8h", 8 * 60), " ", buttonNextAttemptAdd("16h", 16 * 60), " "), dom.br(), dom.div("Now plus"), dom.div(buttonNextAttemptSet("1m", 1), " ", buttonNextAttemptSet("5m", 5), " ", buttonNextAttemptSet("30m", 30), " ", buttonNextAttemptSet("1h", 60), " ", buttonNextAttemptSet("2h", 2 * 60), " ", buttonNextAttemptSet("4h", 4 * 60), " ", buttonNextAttemptSet("8h", 8 * 60), " ", buttonNextAttemptSet("16h", 16 * 60), " ")));
	...
	var hooksRetiredList = async () => {
		...
		return dom.div(
			...
			dom.table(dom._class("hover"), style({ width: "100%" }), dom.thead(dom.tr(dom.td("Filter"), dom.td(), dom.td(filterLastActivity = dom.input(attr.form("hooksfilter"), style({ width: "7em" }), attr.title('Example: ">-1h" for filtering last activity for webhooks more than 1 hour ago.'))), dom.td(filterSubmitted = dom.input(attr.form("hooksfilter"), style({ width: "7em" }), attr.title('Example: "<-1h" for filtering webhooks submitted more than 1 hour ago.'))), dom.td(), dom.td(), dom.td(), dom.td(filterAccount = dom.input(attr.form("hooksfilter"), style({ width: "8em" }))), dom.td(filterEvent = dom.select(
				...
				sortElem = dom.select(attr.form("hooksfilter"), function change() {
					filterForm.requestSubmit();
				}, dom.option("Last activity \u2193", attr.value("nextattempt-desc")), dom.option("Last activity \u2191", attr.value("nextattempt-asc")), dom.option("Submitted \u2193", attr.value("submitted-desc")), dom.option("Submitted \u2191", attr.value("submitted-asc"))),
				" ",
				dom.submitbutton("Apply", attr.form("hooksfilter")),
				" ",
				dom.clickbutton("Reset", attr.form("hooksfilter"), function click() {
	...
	var webserver = async () => {
		...
		const makeHeaders = (h) => {
			...
			const headerRow = (k, v) => {
				...
				let key;
				let value;
				const root2 = dom.tr(dom.td(key = dom.input(attr.required(""), attr.value(k))), dom.td(value = dom.input(attr.required(""), attr.value(v))), dom.td(dom.clickbutton("Remove", function click() {
					rows = rows.filter((x) => x !== row);
					row.root.remove();
					norow.style.display = rows.length ? "none" : "";
				})));
				const get2 = () => [row.key.value, row.value.value];
		...
		const handlerRow = (wh) => {
			...
			const makeWebForward = (wf) => {
				...
				const root2 = dom.table(dom.tr(dom.td("Type"), dom.td("StripPath", attr.title("Strip the matching WebHandler path from the WebHandler before forwarding the request.")), dom.td("URL", attr.title("URL to forward HTTP requests to, e.g. http://127.0.0.1:8123/base. If StripPath is false the full request path is added to the URL. Host headers are sent unmodified. New X-Forwarded-{For,Host,Proto} headers are set. Any query string in the URL is ignored. Requests are made using Go's net/http.DefaultTransport that takes environment variables HTTP_PROXY and HTTPS_PROXY into account. Websocket connections are forwarded and data is copied between client and backend without looking at the framing. The websocket 'version' and 'key'/'accept' headers are verified during the handshake, but other websocket headers, including 'origin', 'protocol' and 'extensions' headers, are not inspected and the backend is responsible for verifying/interpreting them.")), dom.td(dom.span("Response headers", attr.title("Headers to add to the response. Useful for adding security- and cache-related headers.")), " ", responseHeaders.add)), dom.tr(dom.td(dom.select(attr.required(""), dom.option("Static"), dom.option("Redirect"), dom.option("Forward", attr.selected("")), dom.option("Internal"), function change(e) {
					makeType(e.target.value);
				})), dom.td(stripPath = dom.input(attr.type("checkbox"), wf.StripPath || wf.StripPath === void 0 ? attr.checked("") : [])), dom.td(url = dom.input(attr.required(""), attr.placeholder("http://127.0.0.1:8888"), attr.value(wf.URL || ""))), dom.td(responseHeaders)));
				view = { root: root2, get: get2 };
				return view;
			};
			const makeWebInternal = (wi) => {
				...
				const root2 = dom.table(dom.tr(dom.td("Type"), dom.td("Base path", attr.title("Path to use as root of internal service, e.g. /webmail/.")), dom.td("Service")), dom.tr(dom.td(dom.select(attr.required(""), dom.option("Static"), dom.option("Redirect"), dom.option("Forward"), dom.option("Internal", attr.selected("")), function change(e) {
					makeType(e.target.value);
			...
			const moveHandler = (row2, oindex, nindex) => {
				row2.root.remove();
