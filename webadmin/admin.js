...
(function (api) {
	...
	api.types = {
		...
		"ARCCheckResult": { "Name": "ARCCheckResult", "Docs": "", "Fields": [{ "Name": "SealEnabled", "Docs": "", "Typewords": ["bool"] }, { "Name": "SealSelector", "Docs": "", "Typewords": ["string"] }, { "Name": "TrustedSealers", "Docs": "", "Typewords": ["[]", "string"] }, { "Name": "Errors", "Docs": "", "Typewords": ["[]", "string"] }, { "Name": "Warnings", "Docs": "", "Typewords": ["[]", "string"] }, { "Name": "Instructions", "Docs": "", "Typewords": ["[]", "string"] }] },
		...
		"ARC": { "Name": "ARC", "Docs": "", "Fields": [{ "Name": "SealEnabled", "Docs": "", "Typewords": ["bool"] }, { "Name": "SealSelector", "Docs": "", "Typewords": ["string"] }, { "Name": "TrustedSealers", "Docs": "", "Typewords": ["[]", "string"] }] },
	...
	api.parser = {
		...
		ARCCheckResult: (v) => api.parse("ARCCheckResult", v),
	...
	class Client {
		...
		async DomainARCSave(domainName, sealEnabled, sealSelector, trustedSealers) {
			const fn = "DomainARCSave";
			const paramTypes = [["string"], ["bool"], ["string"], ["[]", "string"]];
			const returnTypes = [];
			const params = [domainName, sealEnabled, sealSelector, trustedSealers];
			return await _sherpaCall(this.baseURL, this.authState, { ...this.options }, paramTypes, returnTypes, fn, params);
		}
...
const domain = async (d) => {
	...
	return dom.div(crumbs(crumblink('Mox Admin', '#'), 'Domain ' + domainString(dnsdomain)), domainConfig.Disabled ? dom.p(box(yellow, 'Warning: Domain is disabled. Incoming/outgoing messages involving this domain are rejected and ACME for new TLS certificates is disabled.')) : [], dom.ul(dom.li(dom.a('Required DNS records', attr.href('#domains/' + d + '/dnsrecords'))), dom.li(dom.a('Check current actual DNS records and domain configuration', attr.href('#domains/' + d + '/dnscheck')))), dom.br(), dom.h2('Client configuration'), dom.p('If autoconfig/autodiscover does not work with an email client, use the settings below for this domain. Authenticate with email address and password. ', dom.span('Explicitly configure', attr.title('To prevent authentication mechanism downgrade attempts that may result in clients sending plain text passwords to a MitM.')), ' the first supported authentication mechanism: SCRAM-SHA-256-PLUS, SCRAM-SHA-1-PLUS, SCRAM-SHA-256, SCRAM-SHA-1, CRAM-MD5.'), dom.table(dom.thead(dom.tr(dom.th('Protocol'), dom.th('Host'), dom.th('Port'), dom.th('Listener'), dom.th('Note'))), dom.tbody((clientConfigs.Entries || []).map(e => dom.tr(dom.td(e.Protocol), dom.td(domainString(e.Host)), dom.td('' + e.Port), dom.td('' + e.Listener), dom.td('' + e.Note))))), dom.br(), dom.h2('DMARC aggregate reports summary'), renderDMARCSummaries(dmarcSummaries || []), dom.br(), dom.h2('TLS reports summary'), renderTLSRPTSummaries(tlsrptSummaries || []), dom.br(), dom.h2('Addresses'), dom.table(dom.thead(dom.tr(dom.th('Address'), dom.th('Account'), dom.th('Action'))), dom.tbody(Object.entries(localpartAccounts).map(t => dom.tr(dom.td(prewrap(t[0]) || '(catchall)'), dom.td(dom.a(t[1], attr.href('#accounts/l/' + t[1]))), dom.td(dom.clickbutton('Remove', async function click(e) {
	...
	})(), dom.br(), dom.h2('ARC', attr.title('ARC (Authenticated Received Chain, RFC 8617) allows preserving authentication results across mail forwarding hops. Configure trusted sealers to override DMARC failures from those sealers, and optionally enable ARC sealing for messages forwarded through this domain.')), (() => {
		let fieldset;
		let sealEnabled;
		let sealSelector;
		let trustedSealers;
		...
		}, fieldset = dom.fieldset(dom.div(style({ marginBottom: '1ex', fontStyle: 'italic' }), 'ARC allows trusted intermediaries (spam filters, forwarders) to preserve authentication results across hops, preventing DMARC failures caused by message modifications during forwarding.'), dom.div(style({ display: 'flex', gap: '2em', flexWrap: 'wrap' }), dom.label(style({ display: 'block', marginBottom: '1ex' }), dom.div('Enable ARC sealing', attr.title('When enabled, messages forwarded through this domain will have ARC headers added.')), dom.div(sealEnabled = dom.input(attr.type('checkbox'), arcConf?.SealEnabled ? attr.checked('') : []))), dom.label(style({ display: 'block', marginBottom: '1ex' }), dom.div('Seal selector', attr.title('DKIM selector to use for ARC sealing. Must be a selector configured in the DKIM section.')), dom.div(sealSelector = dom.select(dom.option(''), ...selectorNames.map(name => dom.option(name, arcConf?.SealSelector === name ? attr.selected('') : [])))))), dom.label(style({ display: 'block', marginBottom: '1ex' }), dom.div('Trusted ARC sealers', attr.title('Domains whose ARC seals are trusted for overriding DMARC failures. One domain per line.')), dom.div(trustedSealers = dom.textarea(attr.rows('4'), style({ width: '100%', maxWidth: '30em' }), (arcConf?.TrustedSealers || []).join('\n')))), dom.div(dom.submitbutton('Save ARC configuration'))));
...
const domainDNSCheck = async (d) => {
	...
	return dom.div(crumbs(crumblink('Mox Admin', '#'), crumblink('Domain ' + domainString(dnsdomain), '#domains/' + d), 'Check DNS'), dom.h1('DNS records and domain configuration check'), resultSection('DNSSEC', checks.DNSSEC, detailsDNSSEC), resultSection('IPRev', checks.IPRev, detailsIPRev), resultSection('MX', checks.MX, detailsMX), resultSection('TLS', checks.TLS, detailsTLS), resultSection('DANE', checks.DANE, detailsDANE), resultSection('SPF', checks.SPF, detailsSPF), resultSection('DKIM', checks.DKIM, detailsDKIM), resultSection('DMARC', checks.DMARC, detailsDMARC), resultSection('ARC', checks.ARC, checks.ARC.SealEnabled ? [dom.div('Sealing enabled with selector: ' + checks.ARC.SealSelector), (checks.ARC.TrustedSealers || []).length > 0 ? dom.div('Trusted sealers: ' + (checks.ARC.TrustedSealers || []).join(', ')) : []] : []), resultSection('Host TLSRPT', checks.HostTLSRPT, detailsTLSRPT(checks.HostTLSRPT)), resultSection('Domain TLSRPT', checks.DomainTLSRPT, detailsTLSRPT(checks.DomainTLSRPT)), resultSection('MTA-STS', checks.MTASTS, detailsMTASTS), resultSection('SRV conf', checks.SRVConf, detailsSRVConf), resultSection('Autoconf', checks.Autoconf, detailsAutoconf), resultSection('Autodiscover', checks.Autodiscover, detailsAutodiscover), dom.br());
};
